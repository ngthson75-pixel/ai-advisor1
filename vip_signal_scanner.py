...
def get_vip_signals_from_db(db_session, limit: int = 50) -> list:
    """
    ...
    """
    try:
        # Try query từ bảng trading_signals hoặc signals
        # Thử với bảng tên 'trading_signals' trước
        try:
...
def init_vip_signal_routes(app, engine, Session):
    ...
    def get_vip_signals():
        """
        GET /api/vip/signals?limit=50&action=BUY&vn30_only=true
        ...
        session = Session()
        try:
            signals = get_vip_signals_from_db(session, limit=limit * 3)
