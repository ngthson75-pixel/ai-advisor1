...
def view_sell_signals():
    """Xem SELL signals từ file (ưu tiên V5.2, fallback V3, cuối cùng DB)"""
    print(f"\n{'='*80}")
    ...
    if not sell_file:
        print("  ⚠️ Chưa chạy SELL scanner hoặc không có file sell_signals*.json")
    ...
    if skipped:
        print(f"  ⏳ Skip T+2: {len(skipped)} ({', '.join(skipped[:5])}{'...' if len(skipped) > 5 else ''})")
    
    if not sells:
        print("\n  ✅ Không có SELL signal — thị trường chưa chạm điều kiện bán")
        return
    
    print(f"\n{'#':>3} {'Ticker':<8} {'Reason':<20} {'Entry':>10} {'Exit':>10} {'P/L':>8} {'Bán':>5}")
    print("-" * 75)
    
    for i, s in enumerate(sells, 1):
        emoji = '🔴' if s.get('profit_loss_pct', 0) < 0 else '🟢'
        reason = s.get('exit_reason', 'UNKNOWN')
        print(f"{i:>3} {s['ticker']:<8} {reason:<20} {s['entry_price']:>10,.0f} {s['exit_price']:>10,.0f} {emoji}{s['profit_loss_pct']:>+7.2f}% {s['exit_quantity_pct']:>4}%")
...
def push_sell_signals_to_production():
    """Push SELL signals từ file lên production (ưu tiên V5.2)"""
    # Ưu tiên V5.2
    sell_file = None
    version = None
    ...
    if not sell_file:
        print("  ❌ Chưa có file sell_signals*.json")
        print("  💡 Chạy: python sell_signal_scanner_v5.2.py")
        return
    ...
    for s in sells:
        emoji = '🔴' if s.get('profit_loss_pct', 0) < 0 else '🟢'
        print(f"     {emoji} {s['ticker']} — {s['exit_reason']} | P/L: {s['profit_loss_pct']:+.2f}% | Bán: {s['exit_quantity_pct']}%")
    ...
    for s in sells:
        ...
        exit_qty = s.get('exit_quantity_pct', 100)
        ...
        payload = {
            ...
            'profit_loss_pct': s.get('profit_loss_pct', 0),
            'exit_quantity_pct': exit_qty,
            'strength': 80,
            'stock_type': s.get('stock_type', 'Mid Cap'),
            'rsi': 0,
            'date': s.get('date', datetime.now().strftime('%Y-%m-%d')),
            'signal_code': s.get('signal_code', f"SELL-{s['ticker']}-{datetime.now().strftime('%Y%m%d')}"),
        ...
        try:
            ...
            if resp.status_code in [200, 201]:
                print(f"     ✅ {s['ticker']} — {s['exit_reason']} ({exit_qty}%)")
                success += 1
            else:
