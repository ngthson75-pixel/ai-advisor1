...
def signal_exists_on_server(api_url, ticker, date, action='BUY'):
    """Kiểm tra signal đã tồn tại trên server chưa — tránh duplicate khi push"""
    try:
        resp = requests.get(f"{api_url}/signals", timeout=10)
        if resp.status_code == 200:
            signals = resp.json().get('signals', [])
            for s in signals:
                if (s.get('ticker', '').upper() == ticker.upper() and
                    str(s.get('date', '')) == str(date) and
                    s.get('action', 'BUY').upper() == action.upper()):
...
def push_vip_signals_to_dashboard():
    ...
    for r in signals_to_push:
        ...
        try:
            # Chống duplicate: check trước khi push
            _sig_date = payload.get('date', datetime.now().strftime('%Y-%m-%d'))
            _exists, _ex_id = signal_exists_on_server(api_url, payload['ticker'], _sig_date)
            if _exists:
                skipped += 1
                print(f"    ⏭️  {payload['ticker']} {_sig_date} đã có (ID={_ex_id})")
                continue
            resp = requests.post(f"{api_url}/signals", json=payload, timeout=30)
...
def add_vip_signal_manual():
    ...
    try:
        ...
        for env_name, api_url in envs:
            # Chống duplicate: kiểm tra trước khi push
            _exists_m, _ex_id_m = signal_exists_on_server(api_url, ticker, signal_date)
            if _exists_m:
                print(f"  ⏭️  {ticker} ngày {signal_date} đã tồn tại trên {env_name} (ID={_ex_id_m}) — bỏ qua")
                continue
            try:
                resp = requests.post(f"{api_url}/signals", json=payload, timeout=30)
                if resp.status_code in (200, 201):
                    ...
                    print(f"  ⚠️  {ticker} đã có trên {env_name} (duplicate — server báo 409)")
                else:
