...
if __name__ == "__main__":
    ...
    # ─── Gửi Telegram VIP sau khi scan xong ──────────────────
    import os as _os, requests as _req
    _admin_key = _os.getenv('ADMIN_SECRET', '')
    _backend   = _os.getenv('BACKEND_URL', 'https://ai-advisor1-backend.onrender.com')
    if signals and _admin_key:
        ...
        VN30 = {'ACB','BCM','BID','BVH','CTG','FPT','GAS','GVR','HDB','HPG',
                ...
                'VRE','TCH','KDH','DXG','VND'}
        vn30_sigs = [s for s in signals if s.get('ticker','').upper() in VN30]
        top5 = sorted(signals, key=lambda x: x.get('strength', 0), reverse=True)[:5]
        lines = [f"📊 <b>Tín hiệu MUA mới — {_dt.now().strftime('%d/%m/%Y')}</b>\n"]
        if vn30_sigs:
            tickers = ', '.join(s['ticker'] for s in vn30_sigs[:8])
            lines.append(f"💎 <b>VN30 MUA: {len(vn30_sigs)} mã</b>\n   {tickers}\n")
        lines.append(f"📈 Tổng MUA hôm nay: <b>{len(signals)}</b>\n")
        lines.append("🏆 <b>Top tín hiệu mạnh:</b>")
        for s in top5:
            star     = '💎' if s.get('ticker','').upper() in VN30 else '📌'
            strength = f" ({s['strength']:.0f}%)" if s.get('strength') else ''
            lines.append(f"  {star} <b>{s['ticker']}</b>{strength}")
        lines.append("\n👉 <a href='https://ai-advisor.vn'>Xem trên app</a>")
        message = "\n".join(lines)
        try:
            resp = _req.post(
                f'{_backend}/api/admin/telegram/broadcast',
                headers={'X-Admin-Key': _admin_key},
                data={'body': message}, timeout=20,
            )
            d = resp.json()
            if d.get('success'):
                logger.info(f"📱 Telegram VIP: {d.get('stats',{}).get('sent',0)} người nhận")
            else:
                logger.warning(f"⚠️ Telegram: {d}")
        except Exception as e:
            logger.warning(f"⚠️ Telegram lỗi: {e}")
    elif not _admin_key:
        logger.info("ℹ️ Bỏ qua Telegram (chưa set ADMIN_SECRET)")
