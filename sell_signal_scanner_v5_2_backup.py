...
def _send_sell_telegram(sell_signals: list, api_url: str):
    """Gửi Telegram notification đến VIP users khi có SELL signal mới."""
    import os, requests as _req
    admin_key = os.getenv('ADMIN_SECRET', '')
    backend   = api_url.replace('/api', '')
    if not admin_key:
        print("⚠️ Chưa set ADMIN_SECRET → bỏ qua Telegram")
        return
    lines = ["📉 <b>Tín hiệu BÁN mới — AI Advisor VIP</b>\n"]
    ...
    for s in sell_signals[:5]:
        pct   = s.get('profit_loss_pct', 0)
        emoji = '🟢' if pct >= 0 else '🔴'
        star  = '💎' if s.get('ticker','').upper() in VN30 else ''
        lines.append(
            f"{emoji}{star} <b>{s['ticker']}</b> — {s.get('exit_reason','')}\n"
            f"   {s['entry_price']:,.0f} → {s['exit_price']:,.0f} | "
            f"<b>{pct:+.1f}%</b> | Bán {s.get('exit_quantity_pct',100)}%"
        )
    if len(sell_signals) > 5:
        lines.append(f"\n...và {len(sell_signals)-5} tín hiệu khác")
    lines.append("\n👉 <a href='https://ai-advisor.vn'>Xem chi tiết trên app</a>")
    message = "\n".join(lines)
    try:
        resp = _req.post(
            f'{backend}/api/admin/telegram/broadcast',
            headers={'X-Admin-Key': admin_key},
            data={'body': message}, timeout=20,
        )
        d = resp.json()
        if d.get('success'):
            print(f"📱 Telegram VIP: {d.get('stats',{}).get('sent',0)} người nhận")
        else:
            print(f"⚠️ Telegram: {d}")
    except Exception as e:
    ...
    if sell_signals:
        ...
        if not args.dry_run:
            ...
            if confirm == 'y':
                ...
                if success > 0:
                    _send_sell_telegram(sell_signals, api_url)
        else:
            print("\n⚠️ DRY RUN")
